...
class RequesterRequisitionTest extends TestCase
{
    ...
    public function test_requester_can_submit_campuran_requisition_with_both_operasional_and_modal_items(): void
    {
        ...
        $requisition = Requisition::where('nomor_surat_unit', '099/FAR/CAMPURAN/2026')->first();
        $this->assertNotNull($requisition);
        $this->assertCount(2, $requisition->requisitionDetails);

        $opDetail = $requisition->requisitionDetails()->where('jenis_belanja', 'Operasi')->first();
        $this->assertNotNull($opDetail);
        $this->assertEquals($this->rbaOperasiBlud->id, $opDetail->rba_account_id);
        $this->assertEquals(750000, $opDetail->subtotal);

        $modDetail = $requisition->requisitionDetails()->where('jenis_belanja', 'Modal')->first();
        $this->assertNotNull($modDetail);
