import { Head } from '@inertiajs/react';

const formatRupiah = (value) =>
    new Intl.NumberFormat('id-ID', {
        style: 'currency',
        currency: 'IDR',
        minimumFractionDigits: 0,
        maximumFractionDigits: 0,
    }).format(Number(value || 0));

const formatTanggal = (dateString) => {
    if (!dateString) return '-';
    const date = new Date(dateString);
    return new Intl.DateTimeFormat('id-ID', {
        day: 'numeric',
        month: 'long',
        year: 'numeric',
    }).format(date);
};

export default function PrintRequisition({ requisition }) {
    const details = requisition.requisition_details || requisition.requisitionDetails || [];

    const grandTotal = details.reduce((acc, item) => {
        const qty = Number(item.quantity_approved ?? item.quantity_requested ?? 0);
        const price = Number(item.unit_price || 0);
        return acc + (qty * price);
    }, 0);

    return (
        <div className="bg-white text-black min-h-screen p-8 max-w-4xl mx-auto font-sans print:p-0 print:max-w-none">
            <Head title={`Cetak - ${requisition.requisition_number}`} />

            {/* Print Action Bar (Hidden during Print) */}
            <div className="print:hidden flex items-center justify-between bg-slate-100 p-4 rounded-xl mb-6 border border-slate-200">
                <div className="flex items-center gap-2">
                    <span className="text-sm font-bold text-slate-700">Dokumen Nota Pengajuan Belanja (E-BLUD)</span>
                </div>
                <div className="flex items-center gap-3">
                    <button
                        type="button"
                        onClick={() => window.history.back()}
                        className="bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 px-4 py-2 rounded-lg text-sm font-semibold transition"
                    >
                        Kembali
                    </button>
                    <button
                        type="button"
                        onClick={() => window.print()}
                        className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-5 py-2 rounded-lg text-sm shadow-sm transition flex items-center gap-2"
                    >
                        <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M6.72 13.829c-.24-1.077-.32-2.14-.32-3.193 0-5.18 4.02-9.386 8.974-9.386 4.954 0 8.973 4.207 8.973 9.386 0 1.053-.08 2.116-.32 3.193M12 18v-4.5m0 0l-2.25 2.25M12 13.5l2.25 2.25M3.75 19.5h16.5" />
                        </svg>
                        Cetak Sekarang
                    </button>
                </div>
            </div>

            {/* Kop Surat (Header) */}
            <div className="flex items-center gap-6 border-b-4 border-black pb-4 mb-6">
                <div className="shrink-0">
                    <img
                        src="/image/logo-vertikal-rsj.png"
                        alt="Logo RSJ Tampan"
                        className="h-24 w-auto object-contain"
                    />
                </div>
                <div className="flex-1 text-center">
                    <h2 className="text-sm font-bold tracking-wider uppercase text-black">
                        PEMERINTAH PROVINSI RIAU
                    </h2>
                    <h1 className="text-xl sm:text-2xl font-black uppercase text-black tracking-tight leading-tight mt-0.5">
                        RUMAH SAKIT JIWA TAMPAN
                    </h1>
                    <p className="text-xs text-black font-medium mt-0.5">
                        Jl. H.R. Soebrantas Km. 12,5 Pekanbaru - Riau | Telp: (0761) 63240
                    </p>
                    <div className="mt-2 pt-1 border-t border-black">
                        <h3 className="text-base font-black tracking-wider uppercase">
                            NOTA PENGAJUAN BELANJA E-BLUD
                        </h3>
                        <p className="text-xs font-bold tracking-wider">
                            NOMOR: {requisition.requisition_number}
                        </p>
                    </div>
                </div>
            </div>

            {/* Body Information Grid */}
            <div className="grid grid-cols-2 gap-4 text-sm mb-6 border border-black p-4 rounded-xs">
                <div>
                    <table className="w-full">
                        <tbody>
                            <tr>
                                <td className="font-bold py-1 w-36">Unit / Divisi</td>
                                <td className="py-1">: {requisition.division?.name || '-'}</td>
                            </tr>
                            <tr>
                                <td className="font-bold py-1">Kode Divisi</td>
                                <td className="py-1">: {requisition.division?.division_code || '-'}</td>
                            </tr>
                            <tr>
                                <td className="font-bold py-1">Nama Pemohon</td>
                                <td className="py-1">: {requisition.user?.name || '-'}</td>
                            </tr>
                            <tr>
                                <td className="font-bold py-1">Klasifikasi Belanja</td>
                                <td className="py-1">: Belanja {requisition.jenis_belanja || 'Operasi'}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div>
                    <table className="w-full">
                        <tbody>
                            <tr>
                                <td className="font-bold py-1 w-36">Tanggal Pengajuan</td>
                                <td className="py-1">: {formatTanggal(requisition.submission_date)}</td>
                            </tr>
                            <tr>
                                <td className="font-bold py-1">Rekening RBA</td>
                                <td className="py-1">: {requisition.rba_account ? `[${requisition.rba_account.account_code}] ${requisition.rba_account.account_name}` : '-'}</td>
                            </tr>
                            <tr>
                                <td className="font-bold py-1">Sumber Dana</td>
                                <td className="py-1">: {requisition.sumber_dana || requisition.rba_account?.sumber_dana || 'BLUD RSJ Tampan'}</td>
                            </tr>
                            <tr>
                                <td className="font-bold py-1">Status Dokumen</td>
                                <td className="py-1">: <span className="font-bold uppercase">{requisition.status?.replace('_', ' ')}</span></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            </div>

            {/* Rincian Barang Table */}
            <div>
                <table className="w-full border-collapse border border-black mt-4 text-sm">
                    <thead>
                        <tr className="bg-gray-100 print:bg-transparent">
                            <th className="border border-black p-2 text-center w-10 font-bold">No</th>
                            <th className="border border-black p-2 text-left font-bold">Nama Barang</th>
                            <th className="border border-black p-2 text-left font-bold">Spesifikasi</th>
                            <th className="border border-black p-2 text-center w-20 font-bold">Satuan</th>
                            <th className="border border-black p-2 text-center w-24 font-bold">Qty Disetujui</th>
                            <th className="border border-black p-2 text-right w-32 font-bold">Harga Satuan</th>
                            <th className="border border-black p-2 text-right w-36 font-bold">Subtotal</th>
                        </tr>
                    </thead>
                    <tbody>
                        {details.map((detail, index) => {
                            const itemName = detail.item?.name || '-';
                            const itemSpec = detail.item?.specification || '-';
                            const itemUnit = detail.item?.unit_type || detail.item?.unit || 'Unit';
                            const qtyApproved = detail.quantity_approved ?? detail.quantity_requested;
                            const price = Number(detail.unit_price || 0);
                            const lineSubtotal = Number(detail.subtotal || qtyApproved * price);

                            return (
                                <tr key={detail.id || index}>
                                    <td className="border border-black p-2 text-center">{index + 1}</td>
                                    <td className="border border-black p-2 font-bold">{itemName}</td>
                                    <td className="border border-black p-2 text-xs">{itemSpec}</td>
                                    <td className="border border-black p-2 text-center">{itemUnit}</td>
                                    <td className="border border-black p-2 text-center font-bold">{qtyApproved}</td>
                                    <td className="border border-black p-2 text-right">{formatRupiah(price)}</td>
                                    <td className="border border-black p-2 text-right font-bold">{formatRupiah(lineSubtotal)}</td>
                                </tr>
                            );
                        })}
                    </tbody>
                    <tfoot>
                        <tr className="bg-gray-100 print:bg-transparent font-bold">
                            <td colSpan={6} className="border border-black p-2 text-right font-black">
                                GRAND TOTAL ANGGARAN:
                            </td>
                            <td className="border border-black p-2 text-right font-black">
                                {formatRupiah(requisition.total_amount || grandTotal)}
                            </td>
                        </tr>
                    </tfoot>
                </table>
            </div>

            {/* Footer / Tanda Tangan (3 Columns) */}
            <div className="mt-16 grid grid-cols-3 gap-4 text-center text-sm">
                {/* Left: Pemohon / Kepala Divisi */}
                <div className="flex flex-col justify-between">
                    <div>
                        <p className="font-medium">Yang Mengajukan,</p>
                        <p className="font-bold">Kepala Unit / Divisi</p>
                    </div>
                    <div className="h-24" />
                    <div>
                        <p className="font-bold underline uppercase">
                            {requisition.user?.name || '....................................'}
                        </p>
                        <p className="text-xs text-slate-600">NIP. ....................................</p>
                    </div>
                </div>

                {/* Center: Bagian Perencanaan */}
                <div className="flex flex-col justify-between">
                    <div>
                        <p className="font-medium">Mengetahui,</p>
                        <p className="font-bold">Bagian Perencanaan & Pengadaan</p>
                    </div>
                    <div className="h-24" />
                    <div>
                        <p className="font-bold underline uppercase">
                            ( .................................... )
                        </p>
                        <p className="text-xs text-slate-600">NIP. ....................................</p>
                    </div>
                </div>

                {/* Right: Bagian Keuangan */}
                <div className="flex flex-col justify-between">
                    <div>
                        <p className="font-medium">Menyetujui,</p>
                        <p className="font-bold">Bagian Keuangan & Anggaran</p>
                    </div>
                    <div className="h-24" />
                    <div>
                        <p className="font-bold underline uppercase">
                            ( .................................... )
                        </p>
                        <p className="text-xs text-slate-600">NIP. ....................................</p>
                    </div>
                </div>
            </div>

            {/* Print Styling Helper */}
            <style>{`
                @media print {
                    @page {
                        size: A4 portrait;
                        margin: 15mm;
                    }
                    body {
                        background-color: #ffffff !important;
                        color: #000000 !important;
                        -webkit-print-color-adjust: exact;
                        print-color-adjust: exact;
                    }
                }
            `}</style>
        </div>
    );
}
